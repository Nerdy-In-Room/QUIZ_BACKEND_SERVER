...
public class RoomCreateValidation {
    public static void validateTopicId(Long topicId) {
        if (topicId == null || topicId <= 0 || topicId > 4) {
    ...
    public static void validateMaxPeople(Integer maxPeople) {
        if (maxPeople == null || maxPeople < 1 || maxPeople > 8) {
            throw new GeneralErrorException(GeneralErrorCode.INVALID_INPUT, "최대 인원은 8명입니다.");
        }
    }

    public static void validateQuizCount(Integer quizCount) {
        if (quizCount == null || quizCount < 1 || quizCount > 10) {
